...
while i <= 7:
    ...
    while condition_2:
        try:
            Res = np.loadtxt('/home/tomas/Desktop/Labo 6 y 7/Labo-7/Analisis/Analisis histeresis/Mediciones histeresis/results led 0-20/Estacionario %s/res/%s (res).txt' % (i, j), skiprows=1)[:, 1]
            R_temp.append(np.mean(Res))
            R_err_temp.append(np.std(Res))
            data_i = np.loadtxt('/home/tomas/Desktop/Labo 6 y 7/Labo-7/Analisis/Analisis histeresis/Mediciones histeresis/results led 0-20/Estacionario %s/iv/%s (iv).txt' % (i, j))
            I_sipm = data_i[:, 0]
...
plt.xlabel('Temperatura (C)', size = 17)
