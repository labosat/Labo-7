...
for i in range(1, folders + 1):
    #windows
    path = '/home/labosat/Desktop/Finazzi-Ferreira/Labo-7/Codigos/Codigos Estabilidad LED-SiPM/results led histeresis/Estacionario %s/' % i
...
for i in range(1, folders + 1):
    ...
    for j in range(0, len(current_lib)):
        ...
        area = integrate.simps(I_up, V_up) - integrate.simps(I_down, V_down)  
      #  area = integrate.trapz(I_up, V_up) - integrate.trapz(I_down, V_down)  
        hyst.append(area)
      #      integrate.nquad?
#        fit_up = np.polyfit(V_up, I_up, 9, w=I_up_err)
#        fit_down = np.polyfit(V_down, I_down, 9, w=I_down_err)
